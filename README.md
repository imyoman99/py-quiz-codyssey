...
### 배운 점
- **클래스 분리의 이유**: 처음엔 함수만으로 짰는데, Quiz와 QuizGame으로 나누니 "퀴즈 데이터"와 "게임 흐름"이 분리되어 수정이 쉬워졌다.
- **데이터 영속성**: 변수는 프로그램이 끝나면 사라진다는 걸 체감했고, JSON 저장으로 해결하는 과정에서 파일 입출력을 이해했다.
- **Git 브랜치**: ain을 건드리지 않고 새 기능을 실험할 수 있다는 게 브랜치의 핵심임을 알았다.

### 어려웠던 점 & 해결
- `state.json`이 손상된 경우 프로그램이 죽는 문제 → `try/except json.JSONDecodeError`로 잡아서 기본 데이터로 복구하도록 해결.